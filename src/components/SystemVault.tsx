...
export const SystemVault: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-6 lg:p-10 font-sans cursor-default">
      <div className="max-w-[1600px] mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end bg-[#091426] p-8 lg:p-10 rounded-[3rem] shadow-2xl relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-4">
              <span className="bg-blue-600 text-white px-3 py-1 rounded text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2">
                 <ShieldCheck size={12} /> {t('header.module')}
              </span>
            ...
            <h1 className="text-4xl font-black text-white tracking-tighter uppercase leading-none">{t('header.title')}</h1>
          ...
          <div className="flex gap-4 mt-8 md:mt-0 relative z-10">
             ...
             >
               {createMutation.isPending ? t('header.deploying') : t('header.deploy')}
             </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-8">
             ...
             <div className="p-8 lg:p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <div>
                   <h3 className="text-sm font-black text-[#091426] uppercase tracking-[0.2em] flex items-center gap-3">
                      <Key size={18} className="text-blue-600" /> {t('registry.title')}
                   </h3>
                   <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">{t('registry.authority')}</p>
                </div>
                <div className="relative">
                   <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" size={14} />
                   <input className="bg-white border border-slate-200 rounded-xl py-2 pl-9 pr-4 text-[11px] font-bold text-[#091426] outline-none focus:ring-4 focus:ring-blue-50 w-48" placeholder={t('registry.filter')} />
                </div>
             </div>
                   ...
                   <div className="space-y-4">
                      {secrets.map((secret: any) => (
                        <div key={secret.id} className="p-8 bg-white border border-slate-100 rounded-[2.5rem] flex items-center justify-between group hover:shadow-xl hover:border-blue-100 transition-all cursor-default">
                           <div className="flex items-center gap-8">
                              ...
                              <div>
                                 <div className="flex items-center gap-3">
                                    <span className="text-[10px] font-mono font-black text-slate-400">{t('registry.id')}: {secret.id.slice(0, 13)}</span>
                                    ...
                                    }`}>{t(`registry.status.${secret.status.toLowerCase()}`)}</span>
                                 </div>
                           ...
                           <div className="flex items-center gap-3">
                              <button onClick={() => accessMutation.mutate(secret.id)} disabled={accessMutation.isPending} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl text-slate-400 hover:text-blue-600 hover:bg-white transition-all shadow-sm">
                                 <Eye size={18} />
                              </button>
                              <button onClick={() => rotateMutation.mutate(secret.id)} disabled={rotateMutation.isPending} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl text-slate-400 hover:text-slate-900 hover:bg-white transition-all shadow-sm">
          ...
          <div className="col-span-12 lg:col-span-4 space-y-8">
             <div className="bg-white border border-slate-200 p-8 rounded-[3rem] shadow-sm relative overflow-hidden group">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-8">{t('metrics.health_index')}</h4>
                <div className="flex flex-col items-center">
                   <div className="relative w-40 h-40 mb-6 flex items-center justify-center">
                      ...
                      <div className="absolute flex flex-col items-center">
                         ...
                         <span className="text-[8px] font-black text-slate-400 uppercase">{t('metrics.entropy')}</span>
                      </div>
                   </div>
             ...
             <div className="bg-[#091426] p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group">
                <h4 className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] mb-6 flex items-center justify-between">
                   {t('stream.title')} <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />
                </h4>
                ...
                <button className="mt-8 w-full py-4 border border-white/10 hover:bg-white/10 rounded-2xl text-[9px] font-black text-white/40 hover:text-white uppercase tracking-widest transition-all">{t('stream.review')}</button>
             </div>
             ...
             <div className="bg-emerald-50 border border-emerald-100 p-8 rounded-[3rem] flex items-center justify-center text-center group hover:bg-emerald-100 transition-all cursor-default">
                <div>
                   ...
                   <p className="text-[9px] font-bold text-emerald-700 uppercase mt-1 tracking-widest">{t('compliance.structural')}</p>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4 text-[9px] font-mono font-black text-slate-300 uppercase tracking-[0.4em]">
           <p className="flex items-center gap-3">
              <FileLock size={16} className="text-[#091426]" /> {t('footer.state')}
           </p>
           <p className="text-[#091426]">{t('footer.protocol', { days: 89 })}</p>
           <p>EPOCH: {Date.now()}</p>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showSecret && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-6 backdrop-blur-3xl bg-[#091426]/60 font-sans" onClick={() => setShowSecret(null)}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white rounded-[4rem] p-12 lg:p-20 max-w-4xl w-full shadow-2xl relative overflow-hidden" onClick={e => e.stopPropagation()}>
               <div className="text-center space-y-10 relative z-10">
                  ...
                  <div>
                    ...
                    <h2 className="text-3xl font-black text-[#091426] uppercase tracking-tighter italic underline decoration-blue-200 underline-offset-8">{showSecret?.name}</h2>
                  </div>
                  <div className="bg-slate-50 border border-slate-100 p-10 rounded-[3rem] group">
                     ...
                     <p className="font-mono text-lg font-black text-[#091426] break-all group-hover:text-blue-600 transition-colors">{showSecret?.value}</p>
                     <button className="mt-10 px-8 py-4 bg-[#091426] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 flex items-center justify-center gap-3 mx-auto shadow-xl shadow-slate-900/20 transition-all">
                        <Copy size={16} /> {t('modal.copy')}
                     </button>
                  </div>
                  <p className="text-xs font-bold text-red-500 uppercase tracking-widest animate-pulse">{t('modal.warning')}</p>
                  <button onClick={() => setShowSecret(null)} className="text-slate-400 hover:text-[#091426] text-[10px] font-black uppercase tracking-widest transition-colors pt-4">{t('modal.terminate')}</button>
               </div>
            </motion.div>
          </motion.div>
        )}
